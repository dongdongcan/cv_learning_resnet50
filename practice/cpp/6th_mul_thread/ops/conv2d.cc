...
void MyConv2d(void* img_in,
  ...
  imp_os << "#include <thread>\n";
  imp_os << "#include <vector>\n";
  ...
  imp_os << "  auto MulProcess = [&](int pro_id, int co_per_pro) {\n";
  imp_os << "    for (int co_idx = 0; co_idx < co_per_pro; co_idx++) {\n";
  ...
  if (conv_idx == 0) {
    ...
    imp_os << "                auto weight_data = weight[(pro_id * co_per_pro + co_idx) * 49 * 3 + "
              "kh_idx * 7 * 3 + kw_idx * "
              "3 + ci_];\n";
    ...
    imp_os << "                in_vec = _mm256_loadu_ps(&img[hi_index * " << wi * ci
           << " + wi_index * " << ci << " + ci_]);\n";
    imp_os
        << "                weight_vec = _mm256_loadu_ps(&weight[(pro_id * co_per_pro + co_idx) * "
        << kernel * kernel * ci << " +\n";
  ...
  imp_os << "          out[ho_idx * " << wo * co << " + wo_idx * " << co
         << " + (pro_id * co_per_pro + co_idx)] = acc;\n";
  imp_os << "        }\n";
  imp_os << "      }\n";
  imp_os << "    }\n";
  imp_os << "  };\n\n";

  imp_os << "  int pro_num = std::thread::hardware_concurrency();\n";
  imp_os << "  std::vector<std::thread> threads;\n";
  imp_os << "  int co_per_pro = " << co << " / pro_num;\n";
  imp_os << "  for (int i = 0; i < pro_num; i++) {\n";
  imp_os << "    threads.push_back(std::thread(MulProcess, i, co_per_pro));\n";
  imp_os << "  }\n";
  imp_os << "  for (auto& it : threads) {\n";
  imp_os << "    it.join();\n";
  imp_os << "  }\n";
  imp_os << "}\n";
